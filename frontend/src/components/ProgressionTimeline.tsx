  ...
  if (active && payload && payload.length) {
    return (
      <div style={{
        ...
        <p style={{ margin: '0 0 8px 0', color: '#aaa', fontSize: '12px', fontWeight: 'bold' }}>{label}</p>
        {payload.map((entry: any, index: number) => (
          <p key={index} style={{ color: entry.color, margin: '4px 0', fontSize: '13px' }}>
            ...
            {entry.name === 'Weight' && entry.payload.reps && (
              <span style={{ color: '#888', marginLeft: '6px' }}>(x{entry.payload.reps})</span>
            )}
          </p>
        ))}
      </div>
  ...
  return (
    <div className="glass-panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ fontFamily: 'var(--font-display)', color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0, fontSize: '18px' }}>
      ...
      <div style={{ width: '100%', height: '400px', minHeight: '400px', marginTop: '16px' }}>
        {loading ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>No data available for this exercise.</div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 20, right: 20, bottom: 0, left: -20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
              <XAxis dataKey="date" tick={{ fill: '#888', fontSize: 11, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={{ stroke: 'rgba(255,255,255,0.1)' }} />
              <YAxis tick={{ fill: '#888', fontSize: 11, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} />
              ...
              <Line 
                ...
                stroke="#39FF14" 
                strokeWidth={3} 
                dot={{ r: 4, fill: '#12141c', stroke: '#39FF14', strokeWidth: 2 }} 
                activeDot={{ r: 6, fill: '#39FF14', stroke: '#fff' }} 
              />
              <Line 
                type="monotone" 
                dataKey="weight" 
                ...
                strokeWidth={2} 
                strokeDasharray="5 5"
                dot={{ r: 3, fill: '#12141c', stroke: '#FF6B00', strokeWidth: 2 }} 
              />
            </LineChart>
          </ResponsiveContainer>
        )}
